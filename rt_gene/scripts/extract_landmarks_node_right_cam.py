...
class LandmarkMethodROS(LandmarkMethodBase):
    def __init__(self, img_proc=None):
        ...
        self.subject_pub = rospy.Publisher("/right_subjects/images", MSG_SubjectImagesList, queue_size=3)
        self.headpose_publisher = rospy.Publisher("/right_subjects/headpose", MSG_HeadposeList, queue_size=3)
        self.landmark_publisher = rospy.Publisher("/right_subjects/landmarks", MSG_LandmarksList, queue_size=3)
        # multiple person faces publication for visualisation
        self.subject_faces_pub = rospy.Publisher("/right_subjects/faces", Image, queue_size=3)
...
if __name__ == '__main__':
    try:
        rospy.init_node('right_extract_landmarks')
