...
class BlinkEstimatorROS(object):
    def __init__(self, device_id_blink, model_files, threshold):
        ...
        self.blink_publisher = rospy.Publisher("/right_subjects/blink", MSG_BlinkList, queue_size=3)
        if self.viz:
            self.viz_pub = rospy.Publisher(rospy.get_param("~viz_topic", "/right_subjects/blink_images"), Image, queue_size=3)

        self.sub = rospy.Subscriber("/right_subjects/images", MSG_SubjectImagesList, self.callback, queue_size=1,
                                    buff_size=2 ** 24)
...
if __name__ == "__main__":
    try:
        rospy.init_node("right_blink_estimator")
